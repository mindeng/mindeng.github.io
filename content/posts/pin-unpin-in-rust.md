...
lastmod = 2024-03-19T11:55:26+08:00
draft = false
+++
...
-   `Pin<P>` 是一个 `struct`, 用于包装任意的指针类型 `P`, 而 `Unpin` 和 `!Unpin` 都是 `trait` 。
-   `Pin<P>` 是一个智能指针，用于保证其包装的指针 `P` 后面的值不会发生移动（即物理地址保持稳定且有效），前提是其目标类型没有实现 `Unpin` 。
